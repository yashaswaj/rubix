...
import com.qubole.rubix.common.metrics.BookkeeperMetricsReporter;
...
public class TestClusterUtil
{
  ...
  public void testGetMetricReportors()
  {
    ...
    Set<BookkeeperMetricsReporter> reporterSet = getMetricsReportors(conf);
    assertEquals(reporterSet.size(), 2, "Number of reporter not correct");
    assertTrue(reporterSet.contains(BookkeeperMetricsReporter.JMX), "Metrics reporters not resolved correctly");
    assertTrue(reporterSet.contains(BookkeeperMetricsReporter.GANGLIA), "Metrics reporters not resolved correctly");
  }
}
